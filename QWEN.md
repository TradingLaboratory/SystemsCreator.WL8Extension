# QWEN.md — Контекст проекта SystemsCreator.WL8Extension

## 📋 Обзор проекта

**SystemsCreator.WL8Extension** — это проект библиотеки расширений для Wealth-Lab 8, содержащий набор пользовательских индикаторов, методов расширения, position score cards и пример клиентского расширения с дочерним окном.
...
```
SystemsCreator.WL8Extension/
├── WL8ExtensionDemos.sln              # Решение Visual Studio
├── README.md                          # Документация проекта
├── .gitignore                         # Игнорируемые файлы Git
└── WL8ExtensionDemos/
    ├── WL8ExtensionDemos.csproj       # Проект библиотеки
...
| Индикатор | Описание |
|-----------|----------|
| `HiLoLimit` | Беззапаздывающий лимит на основе максимальных/минимальных уровней (Dr.Koch) |
...
- **`WL8DemosClientExtension`** — главное расширение, добавляющее пункт меню в Wealth-Lab 8
...
cd C:\Users\vdv-v\source\repos\SystemsCreator.WL8Extension

# Сборка через dotnet CLI
dotnet build WL8ExtensionDemos.sln --configuration Release
```

### Установка в Wealth-Lab 8

1. **Собрать библиотеку** в режиме Release
2. **Скопировать DLL** из `WL8ExtensionDemos/bin/Release/` в папку установки Wealth-Lab 8:
   ```
...
![Community Indicators](https://user-images.githubusercontent.com/3159496/210844526-3beb14c8-824b-49ba-aa5e-685042ef4e23.png)
...
1. **Создать класс** в папке `Indicators/`:
   ```csharp
   using WealthLab.Core;
   using WealthLab.Indicators;
...
2. **Собрать проект**
3. **Скопировать DLL** в папку Wealth-Lab 8
4. **Перезапустить Wealth-Lab**
...
- **Исходный код**: Примеры в папках `Indicators/`, `ExtensionMethods/`
...
| `ChildWindows/*.xaml.cs` | Дочерние окна WPF (1 файл) |
